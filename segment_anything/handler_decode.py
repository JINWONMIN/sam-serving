...
class ModelHandler(BaseHandler):
    ...
    def preprocess(self, data):
        ...
        if isinstance(self.payload, dict) and "image_embeddings" in self.payload \
            ...
            and "input_box" in self.payload:
            image_embeddings = self.payload['image_embeddings']
        else:
            print("one of image_shape, input_point, input_label, image_embeddings, input_box payload dict keys missing.")
            assert False
    ...
    def initialize(self, context):
        ...
        self.device = torch.device("cuda:" + str(properties.get("gpu_id")) \
                                   if torch.cuda.is_available()  and str(properties.get("gpu_id")) != "None" else "cpu")
    ...
    def inference_point(self, image_embeddings):
        """
        ...
        """
        start = time()
        if self.payload['input_box'] is None:
            onnx_coord = np.concatenate([np.array(self.payload['input_point']), np.array([[0.0, 0.0]])], axis=0)[
                None, :, :
            ]
            onnx_label = np.concatenate([np.array(self.payload['input_label']), np.array([-1])], axis=0)[
                None, :
            ].astype(np.float32)
            
        elif self.payload['input_point'] is None:
            onnx_box_coords = np.array(self.payload['input_box']).reshape(2, 2)
            onnx_box_labels = np.array([2, 3])
            onnx_coord = onnx_box_coords.reshape(-1, 2, 2)
            onnx_label = np.expand_dims(onnx_box_labels, axis=0).astype(np.float32)
            
        elif self.payload['input_point'] is not None and self.payload['input_box'] is not None:
            onnx_box_coords = np.array(self.payload['input_box']).reshape(2, 2)
            onnx_box_labels = np.array([2, 3])
            onnx_coord = np.concatenate([np.array(self.payload['input_point']), onnx_box_coords], axis=0)[
                None, :, :
            ]
            onnx_label = np.concatenate([np.array(self.payload['input_label']), onnx_box_labels], axis=0)[
                None, :
            ].astype(np.float32)
    ...
    def mask_to_json(self, mask, scores):
    ...
    def postprocess(self, masks):
        ...
        return self.pil_image_to_base64(masks)
    ...
    def handle(self, data, context):
        ...
        masks, scores = self.inference_point(model_input)
        ...
        #         multipolygon = self.mask_to_json(mask, scores)
        #         jsons.append(multipolygon)
        #     return [{"status": "success", "jsons": jsons, "confidence_scores": [np_to_py_type(score) for score in scores]}]
        # else:
